            ...
            <table id="edit-info" class="content-table_find">
                ...
                while($listExchangeU = mysql_fetch_array($ex_u)){
                    if($listExchangeU["type"]==1){
                        $u=getUserByProductId($listExchangeU["product_id1"]);
                        echo "<tr><td><img width='50px' height='50px' src=' $u[image]' /></td>";
                        echo "<td><a class='green' href='index.php?controller=exchange&action=viewExchangeWait&id=".$listExchangeU['id']."'>Bạn muốn trao đổi sản phẩm với ".$u["fullname"]."<br>";
